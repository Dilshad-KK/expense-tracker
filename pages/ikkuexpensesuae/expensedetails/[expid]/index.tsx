      ...
      <CommonHeader
        ...
        right={(
          <div className='flex items-center gap-2'>
            <HeaderAction
              href={`/ikkuexpensesuae/expensedetails/${expid}/edit`}
              label="Update"
              tone="success"
          ...
          {expenses?.map(item => (
            <div className="flex items-center rounded-xl bg-base-100/40 p-2" key={item.id}>
              ...
              <div className="flex flex-1 items-end justify-center flex-col flex-shrink-0">
                ...
                <div className={`text-[13px] font-poppinsBold ${item?.type === 'Withdrawal' ? 'text-error' : 'text-success'}`}>{'AED'}&nbsp;{item?.amount}</div>
              </div>
            </div>
          ))}
          </PageSection>
